...
def min_cut_rematerialization_partition(
    ...
    *, num_fwd_outputs
        ...
        if hasattr(node, "target") and node.target in [
            ...
            for arg in flattened_arg_list:
                if arg in primal_inputs:
                    print(f"True, view_chain: {view_chain}")
                    ...
                    if upstream_is_alias:
                        ...
                        print(f"upstream_is_alias: True, view_chain: {view_chain}")
                        return True, view_chain
        return False, []

    # TODO: rename
    def if_primal_input_alias_is_saved_move_the_view_creation_to_bwd_graph(saved_values):
        # Trace lineage of saved nodes that are aliases of graph inputs. Move the entire view chain to BWD graph.
        primal_inputs = list(filter(_is_primal, joint_module.graph.nodes))
        for saved_value in saved_values:
            is_alias, view_chain = is_alias_of_primal_input(full_bw_graph, primal_inputs, saved_value)
            if is_alias:
                print(f"view_chain: {view_chain}")
    ...
    if_primal_input_alias_is_saved_move_the_view_creation_to_bwd_graph(saved_values)
