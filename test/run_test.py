...
def run_ci_sanity_check(test: ShardedTest, test_directory, options):
    assert test.name == "test_ci_sanity_check_fail", f"This handler only works for test_ci_sanity_check_fail, got {test.name}"
    ret_code = run_test(test, test_directory, options, print_log=False)
    # This test should fail
    if ret_code != 1:
        return 1
    # Delete the log files and xmls generated by the test
    for file in glob.glob(str(REPO_ROOT / "test/test-reports/test_ci_sanity_check_fail*.log")):
        os.remove(file)
    for dirname in glob.glob(str(REPO_ROOT / "test/test-reports/**/test_ci_sanity_check_fail")):
